...
family = {}
while True:
    name = input('Enter name or empty string if done: ')
    if name == "":
        break
    age = input('Enter age: ')
    family[name] = int(age)
...
print(f"{brand['name']}'s clients are {','.join(brand['type_of_clothes'])}")
...
# Update method replaced values of the existing keys
...
disney_users_D = {user:i for i, user in enumerate(users) if "i" in user}
print(disney_users_D)
# The characters, which names start with the letter “m” or “p”.
disney_users_E = {i:user for i, user in enumerate(users) if user[0].lower() in ["m", "p"]}
